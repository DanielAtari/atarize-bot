...
def handle_question(question, session, collection, system_prompt, client, intents):
    ...
    # Step 1: Handle greetings naturally - warm welcome once per session, then continue conversation
    if is_greeting(question) and not session.get("intro_given"):
        logger.info(f"[GREETING] First-time greeting detected - providing natural welcome")
        session["intro_given"] = True
        lang = detect_language(question)
        natural_response = get_natural_greeting(lang, question)
        session["history"].append({"role": "assistant", "content": natural_response})
        return natural_response, session
    elif is_greeting(question) and session.get("intro_given"):
        logger.debug(f"[GREETING] Repeat greeting detected - responding naturally and continuing conversation")
        # Respond briefly and naturally to repeat greetings, then continue conversation
        lang = detect_language(question)
        if lang == "he":
            brief_responses = [
                "היי שוב! 😊 במה אוכל לעזור?",
                "שלום! איך אוכל לעזור לכם?", 
                "נעים לשוב ולראות אתכם! מה מעניין אתכם לדעת?"
            ]
        else:
            brief_responses = [
                "Hi again! 😊 What can I help you with?",
                "Hello! How can I assist you?",
                "Nice to see you again! What would you like to know?"
            ]
        
        import random
        natural_followup = random.choice(brief_responses)
        session["history"].append({"role": "user", "content": question})
        session["history"].append({"role": "assistant", "content": natural_followup})
        return natural_followup, session
    else:
        logger.debug(f"[GREETING] Not a greeting - continuing to processing")
    ...
    # 2. Small talk and vague message check (basic responses only)
    logger.debug(f"[SMALL_TALK_CHECK] Checking if message is basic small talk")
    if is_small_talk(question):
        logger.info(f"[SMALL_TALK_CHECK] ✅ Detected basic small talk: '{question}'. Providing simple acknowledgment.")
        lang = detect_language(question)
        if lang == "he":
            return "תודה! 😊", session
        else:
            return "Thank you! 😊", session
    logger.debug(f"[SMALL_TALK_CHECK] ❌ Not basic small talk - continuing")
    
    # Check if question is too short or generic (for user input, not GPT responses)
    if len(question.strip()) < 3:
        logger.info(f"[VAGUE_CHECK] ✅ Question too short: '{question}'. Skipping intent and Chroma.")
        lang = detect_language(question)
        if lang == "he":
            return "אפשר לעזור במשהו מסוים? 😊", session
        else:
            return "Can I help with something specific? 😊", session
    logger.debug(f"[VAGUE_CHECK] ❌ Question length OK - continuing to business type detection")

    # 2. Use case detection - HIGHEST priority for conversion opportunities 
    logger.debug(f"\n[USE_CASE_DETECTION] ======== CHECKING FOR SPECIFIC USE CASES ========")
    specific_use_case = detect_specific_use_case(question)
    if specific_use_case:
        logger.info(f"[USE_CASE_DETECTION] ✅ Specific use case detected: '{specific_use_case}' in question: '{question}'")
        lang = detect_language(question)
        use_case_response = get_use_case_specific_response(specific_use_case, question, lang)
        
        # Mark as VERY HOT lead - they described specific pain points
        session["specific_use_case"] = specific_use_case
        session["business_info"] = question
        session["interested_lead_pending"] = True  # High-quality lead
        session["conversion_opportunity"] = True  # Flag for high-value prospect
        
        # Add to conversation history
        session["history"].append({"role": "user", "content": question})
        session["history"].append({"role": "assistant", "content": use_case_response})
        
        logger.info(f"[USE_CASE_DETECTION] ✅ Generated targeted use case response, marked as high-value lead")
        return use_case_response, session

    # 2b. Business type detection - high priority for conversation flow 
    logger.debug(f"\n[BUSINESS_DETECTION] ======== CHECKING FOR BUSINESS TYPE ========")
    if detect_business_type(question):
        logger.info(f"[BUSINESS_DETECTION] ✅ Business type detected: '{question}'")
        lang = detect_language(question)
        business_response = get_business_specific_response(question, lang)
        
        # Mark as valuable lead and update session state
        session["business_type_provided"] = True
        session["business_info"] = question
        session["interested_lead_pending"] = True  # This person is now a hot lead
        
        # Add to conversation history
        session["history"].append({"role": "user", "content": question})
        session["history"].append({"role": "assistant", "content": business_response})
        
        logger.info(f"[BUSINESS_DETECTION] ✅ Generated targeted response, marked as lead")
        return business_response, session
    
    # 2a. Smart follow-up detection - handle "yes" responses with specific content
    follow_up_topic, business_context = detect_follow_up_context(question, session)
    if follow_up_topic:
        logger.info(f"[FOLLOW_UP] ✅ Follow-up context detected: '{follow_up_topic}'")
        lang = detect_language(question)
        
        # Generate specific follow-up content based on conversation context
        follow_up_content = get_follow_up_content_by_topic(follow_up_topic, business_context, lang)
        
        # Mark as interested lead since they're asking for more details
        ...
        logger.info(f"[FOLLOW_UP] ✅ Generated targeted follow-up content for '{follow_up_topic}'")
        return follow_up_content, session
    
    # 2b. Positive engagement detection - handle "כן", "בטח", etc. (fallback)
    elif detect_positive_engagement(question):
        logger.info(f"[ENGAGEMENT] ✅ Positive engagement detected: '{question}'")
        lang = detect_language(question)
        
        # Check conversation context to understand what they're responding to
        history = session.get("history", [])
        if len(history) >= 2:
            last_bot_message = history[-1].get("content", "").lower()
            
            # If bot asked about business type, prompt for it
            if any(phrase in last_bot_message for phrase in ["איזה סוג עסק", "business type", "תחום עסקי"]):
                if lang == "he":
                    follow_up = "נהדר! אז איזה סוג עסק יש לך? (חנות, מסעדה, קליניקה, משרד, וכו') כך אוכל לתת לך דוגמאות ספציפיות איך הבוט יעזור דווקא לך 😊"
                else:
                    follow_up = "Great! So what type of business do you have? (store, restaurant, clinic, office, etc.) This way I can give you specific examples of how the bot will help you specifically 😊"
            # If bot asked about pricing/cost
            elif any(phrase in last_bot_message for phrase in ["מחיר", "עלות", "price", "cost"]):
                if lang == "he":
                    follow_up = "מעולה! עלות ההקמה היא 600 ש\"ח חד פעמי, ואז תכנית חודשית מ-59 ש\"ח. איזה סוג עסק יש לך? אוכל להסביר איך הבוט יחסוך לך כסף בתחום שלך 💰"
                else:
                    follow_up = "Excellent! Setup cost is 600 ILS one-time, then monthly plan from 59 ILS. What type of business do you have? I can explain how the bot will save you money in your specific field 💰"
            # General positive engagement
            else:
                if lang == "he":
                    follow_up = "מצוין! אני כאן לעזור לך להבין איך הבוט יכול לשפר את העסק שלך. איזה סוג עסק יש לך? יש לי דוגמאות ספציפיות לכל תחום 🎯"
                else:
                    follow_up = "Excellent! I'm here to help you understand how the bot can improve your business. What type of business do you have? I have specific examples for every field 🎯"
        else:
            # No previous context, general engagement response
            if lang == "he":
                follow_up = "נהדר שאתה מתעניין! איזה סוג עסק יש לך? כך אוכל לתת לך דוגמאות ספציפיות איך הבוט יעזור דווקא לך 😊"
            else:
                follow_up = "Great that you're interested! What type of business do you have? This way I can give you specific examples of how the bot will help you specifically 😊"
        
        session["history"].append({"role": "user", "content": question})
        session["history"].append({"role": "assistant", "content": follow_up})
        
        logger.info(f"[ENGAGEMENT] Generated contextual follow-up response")
...
(If your question didn't get a full response, I'd appreciate if you could rephrase it slightly)"""
    ...
    if contextual_intent:
        ...
        # Performance timing - overall request
    ...
    if knowledge_docs:
        ...
        # 4. Build GPT prompt with examples, context, and conversational enhancements
        ...
        if base_tokens + count_tokens([{"role": "system", "content": examples_block}]) < Config.MAX_PROMPT_TOKENS:
            context_prompt = f"{system_prompt}\n\n{examples_block}הקשר רלוונטי מתוך מסמכי העסק:\n{enhanced_context}\n\n{conversational_enhancement}\n\nשאלה של המשתמש:\n{question}\n\nענה בצורה טבעית וידידותית בהתבסס על המידע שלעיל. התאמי את התשובה לשאלה הספציפית, הוסיפי ערך אישי, וסיימי עם שאלת המשך רלוונטית כדי להמשיך את השיחה באופן טבעי."
        ...
        if is_vague_gpt_answer(answer):
            logger.info(f"[ANSWER_VALIDATION] ❌ GPT-4 answer is vague, checking for intelligent fallback.")
            
            # Enhanced fallback logic - check if user provided valuable information that we missed
            specific_use_case = detect_specific_use_case(question)
            if specific_use_case:
                logger.info(f"[ANSWER_VALIDATION] ✅ Vague answer but specific use case detected: '{specific_use_case}'")
                lang = detect_language(question)
                use_case_response = get_use_case_specific_response(specific_use_case, question, lang)
                session["specific_use_case"] = specific_use_case
                session["business_info"] = question
            ...
            elif detect_business_type(question):
                logger.info(f"[ANSWER_VALIDATION] ✅ Vague answer but business type detected, providing specific response")
                lang = detect_language(question)
                business_response = get_business_specific_response(question, lang)
                session["business_type_provided"] = True
                session["business_info"] = question
            ...
            # Check for follow-up context when GPT gives vague answer
            elif detect_follow_up_context(question, session)[0]:
                logger.info(f"[ANSWER_VALIDATION] ✅ Vague answer but follow-up context detected, providing specific content")
                follow_up_topic, business_context = detect_follow_up_context(question, session)
                lang = detect_language(question)
                follow_up_content = get_follow_up_content_by_topic(follow_up_topic, business_context, lang)
                session["interested_lead_pending"] = True
                ...
                logger.info(f"[ANSWER_VALIDATION] ✅ Vague answer but positive engagement detected, guiding conversation")
                lang = detect_language(question)
                if lang == "he":
                    engagement_response = "נהדר! אני רואה שאתה מתעניין. איזה סוג עסק יש לך? כך אוכל לתת לך דוגמאות ספציפיות איך הבוט יעזור דווקא לך 😊"
                else:
                    engagement_response = "Great! I can see you're interested. What type of business do you have? This way I can give you specific examples of how the bot will help you specifically 😊"
                session["history"].append({"role": "assistant", "content": engagement_response})
                return engagement_response, session
            
            # Don't activate lead collection for greetings/small talk even if GPT answer is vague
            elif is_greeting(question) or is_small_talk(question):
                logger.debug(f"[ANSWER_VALIDATION] Vague answer for greeting/small talk - providing intro response instead of lead collection")
                # Detect language and respond appropriately
                lang = detect_language(question)
                return get_natural_greeting(lang, question), session
            
            # Only activate lead collection for truly vague cases with no identifiable value
            else:
                logger.info(f"[ANSWER_VALIDATION] ❌ No valuable patterns detected, activating lead collection")
                session["interested_lead_pending"] = True
